...
## Automated IP Discovery ✨ NEW!

The proxy now supports **fully automated IP discovery**! No more manual copy-paste.

**Quick Setup**:
1. Install Python dependencies: `cd python/akamTester-master && pip install -r requirements.txt`
2. Enable in `config.json5`: Set `akamTester.enabled: true`
3. Start proxy: `npm start`

**Features**:
- Automatically runs akamTester every 15 minutes (configurable)
- Discovers new IPs from global DNS sources
- Merges with existing IPs and tests with tcp-ping
- Removes dead IPs automatically
- No server restart needed!

**See [CLAUDE.md](CLAUDE.md#automated-ip-discovery-akamtester-integration) for detailed documentation.**
